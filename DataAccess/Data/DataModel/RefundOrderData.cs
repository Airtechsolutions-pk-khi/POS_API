            ...
            {                 
                ...
                foreach (var item in order.Items.ToList())
                {
                    if (item.IsSelected == true)
                    {
                        ...
                        //var UPrice = item.Price / item.Quantity;  
                        var p = item.UnitPrice * item.Quantity;
                        var UPrice = p / item.Quantity;
                        ...
                        parm[0] = new SqlParameter("@OrderId", item.ID);
                        parm[1] = new SqlParameter("@LocationID", order.LocationID);
                        parm[2] = new SqlParameter("@RefundQuantity", item.RefundQuantity);
                         new DBHelper().GetTableFromSP("sp_UpdateStockRefund", parm);

                        SqlParameter[] parm1 = new SqlParameter[4];
                        parm1[0] = new SqlParameter("@ItemID", item.ID);
                        parm1[1] = new SqlParameter("@DiscountPrice", item.DiscountPrice);
                        parm1[2] = new SqlParameter("@RefundQuantity", item.RefundQuantity);
                        parm1[3] = new SqlParameter("@RefundPrice", item.RefundPrice);
                ...
                parm2[0] = new SqlParameter("@OrderID", order.ID);
                parm2[1] = new SqlParameter("@LocationID", order.LocationID);
                parm2[2] = new SqlParameter("@RefundAmount", order.RefundAmount);
                parm2[3] = new SqlParameter("@RefundPrice", order.TotalRefund);
