...
namespace DataAccess.Data.DataModel
{
    public class HoldOrderData : IHoldOrderData
    {
        ...
        {
            //DateTime dateAndTime = DateTime.UtcNow.AddMinutes(180);
            ...
            p[8] = new SqlParameter("@OrderCreatedDT", DateTime.UtcNow.AddMinutes(180).Date);
            ...
            if (order.ID > 0)
            {
                try
                {
                    OrderID = int.Parse(new DBHelper().GetTableFromSP("Update_HoldOrder_P_API", p).Rows[0]["OrderID"].ToString());
                    
                    //or = (await _service.LoadData<OrderReturn, dynamic>("[dbo].[Insert_HoldOrder_P_API]", p)).FirstOrDefault();
                }
                catch (Exception ex)
                {
                }
            ...
            foreach (var item in order.Items)
            {
                ...
                try
                {
                    ...
                    {
                        OrderDetailID = int.Parse(new DBHelper().GetTableFromSP("sp_DeleteOrderDetail_P_API", p1).Rows[0]["ID"].ToString());
                    }
            ...
            or.Items = await _service.LoadData<OrderDetail, dynamic>("[dbo].[sp_GetHoldOrderDetailsByOrderId_P_API]", new { OrderID });
            var odm = await _service.LoadData<OrderModifierDetail, dynamic>("[dbo].[sp_GetOrderModifierByOrderId_P_API]", new { OrderID });
        ...
        public async Task<OrderReturn> SaveCreditData(Order<Item> order)
        {
            ...
            {
                or = await _service.SaveSingleQueryable<OrderReturn, dynamic>("[dbo].[sp_InsertCreditOrder_P_API]",
             new { ParamTable1 = JsonConvert.SerializeObject(order) });
