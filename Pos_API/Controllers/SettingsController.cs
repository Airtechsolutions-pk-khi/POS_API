...
namespace Pos_API.Controllers
{
	...
	public class SettingsController : ControllerBase
	{
		...
		public SettingsController(ILogger<SettingsController> logger, ITablesData tabledata, IWaiterData waiterdata, ICustomerData customerdata)
		{
			_logger = logger;
		...
		public async Task<IActionResult> GetAllSettingsData(int LocationID, int UserID)
		{
			...
			var waiters = await _waiterdata.GetAllWaiters(LocationID);
			var tables = await _tabledata.GetAllTables(LocationID);
			var customers = await _customerdata.GetAllCustomers(UserID);
